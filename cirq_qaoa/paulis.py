...
from cirq import Circuit, GridQubit, inverse, InsertStrategy, ZPowGate, Rx, Rz, X, H, CNOT, Z
...
class PauliTerm(object):
    ...
    def __eq__(self, other):
        if not isinstance(other, (PauliTerm, PauliSum)):
            raise TypeError(
                "Can't compare PauliTerm with object of type {}.".format(type(other)))
...
class PauliSum(object):
    ...
    def __init__(self, terms):
        ...
        if not (isinstance(terms, Sequence)
            ...
            raise ValueError(
                "PauliSum's are currently constructed from Sequences of PauliTerms.")
        if len(terms) == 0:
            self.terms = [0.0 * ID()]
        else:
            self.terms = terms
    ...
    def __eq__(self, other):
        ...
        if not isinstance(other, (PauliTerm, PauliSum)):
            raise TypeError(
                "Can't compare PauliSum with object of type {}.".format(type(other)))
        elif isinstance(other, PauliTerm):
            return self == PauliSum([other])
        elif len(self.terms) != len(other.terms):
            warnings.warn(UnequalLengthWarning(
                "These PauliSums have a different number of terms."))
            return False
    ...
    def __mul__(self, other):
        ...
        if not isinstance(other, (Number, PauliTerm, PauliSum)):
            ...
            other_terms = [other]
        new_terms = [lterm * rterm for lterm,
                     rterm in product(self.terms, other_terms)]
...
def is_identity(term):
    ...
    if isinstance(term, PauliTerm):
        ...
        raise TypeError(
            "is_identity only checks PauliTerms and PauliSum objects!")
...
def exponential_map(term):
    ...
    def exp_wrap(param):
        ...
        if is_identity(term):
            ...
            circuit += _exponentiate_general_case(term, param)
        return circuit
...
def _exponentiate_general_case(pauli_term, param):
    """
    Returns a CriQ (Circuit()) object corresponding to the exponential of
    ...
    :returns: A CirQ circuit object
    :rtype: Circuit
    """

    def reverse_hack(p):
        # A hack to produce a *temporary* circuit which reverses p.
        rev_circuit = Circuit()
        operations_in_p = []
        reverse_operations_in_p = []
        for operation in p.all_operations():
            operations_in_p.append(operation)

        reverse_operations_in_p = inverse(operations_in_p)

        for operation in reverse_operations_in_p:
            rev_circuit.append([operation], strategy=InsertStrategy.EARLIEST)
        return rev_circuit
    ...
    for index, op in pauli_term:
        if 'X' == op:
            change_to_z_basis.append(
                [H(index)], strategy=InsertStrategy.EARLIEST)
            ...
            RX = Rx(np.pi/2.0)
            change_to_z_basis.append(
                [RX(index)], strategy=InsertStrategy.EARLIEST)
            RX = inverse(RX)
            change_to_original_basis.append(
                [RX(index)], strategy=InsertStrategy.EARLIEST)
        ...
        if prev_index is not None:
            cnot_seq.append([CNOT(prev_index, index)],
                            strategy=InsertStrategy.EARLIEST)
    ...
    circuit += cnot_seq
    RZ = Rz(2.0 * pauli_term.coefficient *
            param)
    circuit.append([RZ(highest_target_index)],
                   strategy=InsertStrategy.EARLIEST)
    circuit += reverse_hack(cnot_seq)
